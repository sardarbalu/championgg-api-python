...
class Champions(object):
    ...
    def all(self, options=None):
        if options is None:
            options = {}
        if self.__alldata == "":
            r = requests.get("%s/champions?api_key=%s&%s" % (self.__url, self.__key, options))
    ...
    def specific(self, id, options=None):
        ...
        r = requests.get("%s/champions/%d?api_key=%s&%s" % (self.__url, id, self.__key, options))
        return r.json()
    ...
    def specific_role(self, id, role, options=None):
        ...
        r = requests.get("%s/champions/%d/%s?api_key=%s&%s" % (self.__url, id, role, self.__key, options))
        try:
            return r.json()[0]
        except IndexError:
            return None
    ...
    def specific_matchup(self, champ1, champ2, role, options=None):
        ...
        r = requests.get("%s/champions/%d/matchups/%d/%s?api_key=%s&%s" %
                         (self.__url, champ1, champ2, role, self.__key, options))
        return r.json()
